...
!        allocate(wing(99, 2), source=0.0_real64)
		...
		do i = 1, d
			do k = 1, size(data_4D137, 1)
			    S(k,i) = 0.05_8 * cos((1.0_8 - data_4D137(k,i))/2.0_8 * 3.1415926535_8)
			end do
		end do
...
!		real(kind=8), allocatable :: wing_coords(:,:), inoutput_coords(:,:), r_row(:), phi_row(:)
		...
		d_wing_y(:), d_inoutput_x(:), d_inoutput_y(:), weights_x(:), weights_y(:)
...
!		allocate(control_points(nControl,2))
		...
		d_inoutput_x = 0.0_8; d_inoutput_y = 0.0_8
		...
		d_mesh_x = 0.0_8; d_mesh_y = 0.0_8; d_mesh_z = 0.0_8
...
!		allocate(phi(nControl, nControl))
		do j = 1, nControl
			...
			call gaussian_rbf_array(r_row, epsilonn, phi_row(1:nControl), nControl) 
			phi(j,:) = phi_row
		end do
        allocate(weights_x(nControl), source=0.0_8)  ! 预初始化
...
!		allocate(distances(nControl))
		do pt = 1, nPoints
			...
			call gaussian_rbf_array(distances, epsilonn, phi_row(1:nControl), nControl)
...
!		open(newunit=unit, file=filename, status='old')
        unit = get_free_unitt()  ! 用自定义函数获取有效单元号
