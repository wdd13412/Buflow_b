...
!                *fluxresiduals *cellfluxes_sln *cvols_mesh *favecs_mesh
!                *fcenters_mesh *cellprimitives_sln
...
&         diffusionflux(v)*unitfa
      END DO
    END DO
    CALL INTEGRATEFLUXES_UNSTRUCTURED3D_B()
...
&                                   gradp, 2))
    ...
    epssb = 0.0_8
    CALL GREENGAUSSGRADD_B(fdeltas, fdeltasb, .false., fdgrads, fdgradsb&
&                   )
    ...
    DO f=nfaces,1,-1
      DO v=nvars,1,-1
        DO d=3,1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            favecs_meshb(f, d) = favecs_meshb(f, d) - facevals(f, v)*&
&             gradb(neighbourcell, v, d)
            facevalsb(f, v) = facevalsb(f, v) - favecs_mesh(f, d)*gradb(&
&             neighbourcell, v, d)
          END IF
          favecs_meshb(f, d) = favecs_meshb(f, d) + facevals(f, v)*gradb&
&           (ownercell, v, d)
          facevalsb(f, v) = facevalsb(f, v) + favecs_mesh(f, d)*gradb(&
&           ownercell, v, d)
        END DO
    ...
    CALL POPCONTROL2B(branch)
    IF (branch .EQ. 0) THEN
      matrixb = matrixb + facevalsb
      IF (ALLOCATED(fcenters_meshb)) fcenters_meshb = 0.0_8
      facevalsb = 0.0_8
    ELSE IF (branch .EQ. 1) THEN
      IF (ALLOCATED(fcenters_meshb)) fcenters_meshb = 0.0_8
      CALL LININTERP_3D_B(matrix, matrixb, facevals, facevalsb)
    ELSE
      IF (ALLOCATED(fcenters_meshb)) fcenters_meshb = 0.0_8
      CALL LININTERP_3D_B(matrix, matrixb, facevals, facevalsb)
    END IF
...
!  Differentiation of greengaussgradd in reverse (adjoint) mode (with options noISIZE i4 dr8 r4):
!   gradient     of useful results: *ccenters_mesh *facevals *fcenters_mesh
!                matrix
...
!                grad:in
  SUBROUTINE GREENGAUSSGRADD_B(matrix, matrixb, valuesatfaces, grad, &
&   gradb)
    IMPLICIT NONE
!		type(Meshh), intent(in) :: mesh  ! 网格信息
! 单元变量矩阵（行：单元，列：变量）
...
! 若面值未直接提供，则通过线性插值计算面值
    ...
    ELSE
      CALL LININTERP_3D_B(matrix, matrixb, facevals, facevalsb)
    END IF
  END SUBROUTINE GREENGAUSSGRADD_B
...
! 邻居单元格
    ...
    DO f=nfaces-nbdryfaces,1,-1
      ...
      IF (branch .EQ. 0) THEN
        c2 = faces_mesh(f, 2)
        rjb(c2) = rjb(c2) + rjsjfb(f, 2)
        rjsjfb(f, 2) = 0.0_8
        c1 = faces_mesh(f, 1)
      ELSE
        c1 = faces_mesh(f, 1)
        rjb(c1) = rjb(c1) + rjsjfb(f, 2)
        rjsjfb(f, 2) = 0.0_8
        c2 = faces_mesh(f, 2)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        sjb(c2) = sjb(c2) + rjsjfb(f, 1)
        rjsjfb(f, 1) = 0.0_8
      ELSE
        sjb(c1) = sjb(c1) + rjsjfb(f, 1)
        rjsjfb(f, 1) = 0.0_8
      END IF
    END DO
...
!   gradient     of useful results: *fluxresiduals
!   with respect to varying inputs: *fluxresiduals *facefluxes_sln
!                *cvols_mesh *favecs_mesh
...
!   残差 = -∇·F（F为通量矢量）
!=#
  SUBROUTINE INTEGRATEFLUXES_UNSTRUCTURED3D_B()
    IMPLICIT NONE
!		type(Meshh), intent(in) :: mesh
!		type(SolutionState), intent(inout) :: sln
...
!		    if (Uz /= Uz .or. abs(Uz) > INF_THRESHOLD) Uz = 0.0d0
  ...
  SUBROUTINE WRITEOUTPUT_B(cellprimitives, restartfile, meshpath, &
&   createrestartfile, createvtkoutput, point_update, drag_value, dJdp, &
&   dJdp_partial, cellprimitivesoutb)
    IMPLICIT NONE
    REAL(kind=8), INTENT(IN) :: cellprimitives(:, :)
    CHARACTER(len=*), INTENT(IN) :: restartfile
    CHARACTER(len=*), INTENT(IN) :: meshpath
    ...
    LOGICAL, INTENT(IN) :: createvtkoutput
    REAL*8, ALLOCATABLE, INTENT(IN) :: point_update(:, :)
    REAL(kind=8), INTENT(IN) :: drag_value
    REAL(kind=8), INTENT(IN) :: dJdp(4), dJdp_partial(4)
    REAL(kind=8), INTENT(IN) :: cellprimitivesoutb(:, :)
    INTEGER(kind=8) :: unit, iostat
    CHARACTER(len=256) :: solnname
    LOGICAL :: exists
    INTRINSIC TRIM
    ...
    END IF
    IF (createvtkoutput) THEN
      solnname = 'solution_adjoint_001.vtk'
      PRINT*, 'Writing VTK File (adjoint): ', TRIM(solnname)
      CALL OUTPUTVTK_B(meshpath, cellprimitives, solnname, point_update, &
&               drag_value, dJdp, dJdp_partial, cellprimitivesoutb)
    END IF
  END SUBROUTINE WRITEOUTPUT_B

! 2. 修正调用writeVTKFile时的参数传递（确保points为二维数组）
  SUBROUTINE OUTPUTVTK_B(meshpath, cellprimitives, filename, point_update, &
&   drag_value, dJdp, dJdp_partial, cellprimitivesoutb)
    IMPLICIT NONE
    CHARACTER(len=*), INTENT(IN) :: meshpath
    REAL(kind=8), INTENT(IN) :: cellprimitives(:, :), cellprimitivesoutb(:, :)
    REAL(kind=8), INTENT(IN) :: drag_value, dJdp(4), dJdp_partial(4)
    CHARACTER(len=*), INTENT(IN), OPTIONAL :: filename
    REAL*8, ALLOCATABLE, INTENT(IN) :: point_update(:, :)
    REAL(kind=8), ALLOCATABLE :: pointlocations(:, :)
    TYPE(CELLLARRAY) :: cells
...
&                           point_update)
    ...
    IF (PRESENT(filename)) THEN
      vtkfilename = TRIM(filename)//'.vtk'
    ELSE
      vtkfilename = 'solution_adjoint.vtk'
    END IF
    CALL WRITEVTKFILE_B(vtkfilename, pointlocations, cells, cellprimitives, &
&   npoints, ncells, (/1_int64, 3_int64, 5_int64, 10_int64, 14_int64, &
&   13_int64, -1_int64, 12_int64/), drag_value, dJdp, dJdp_partial, &
&   cellprimitivesoutb)
  END SUBROUTINE OUTPUTVTK_B

  SUBROUTINE WRITEVTKFILE_B(filename, points, cells, primitives, npts, &
&   ncells, vtktypes, drag_value, dJdp, dJdp_partial, dJdw)
    IMPLICIT NONE
    CHARACTER(len=*), INTENT(IN) :: filename
    REAL(kind=8), INTENT(IN) :: points(:, :)
    TYPE(CELLLARRAY), INTENT(IN) :: cells
!$AD NONDIFF
    REAL(kind=8), INTENT(IN) :: primitives(:, :), dJdw(:, :)
    REAL(kind=8), INTENT(IN) :: drag_value, dJdp(4), dJdp_partial(4)
    INTEGER(kind=8), INTENT(IN) :: npts, ncells
    INTEGER(kind=8), INTENT(IN) :: vtktypes(8)
    INTEGER(kind=8) :: fileunit, i, j, n, celltype, ptidx, connoffset
    ...
    INTRINSIC SIZE
    INTRINSIC MERGE
    IF (SIZE(primitives, 1) .NE. ncells .OR. SIZE(primitives, 2) .LT. 5) THEN
      PRINT*, 'Error: flow data dim (cells=', ncells, ', rows=', SIZE(primitives, 1), ')'
      RETURN
    END IF
    fileunit = GET_FREE_UNIT()
    OPEN(unit=fileunit, file=filename, status='replace', action='write', form='formatted')
    WRITE(fileunit, '(A)') '# vtk DataFile Version 3.0'
    WRITE(fileunit, '(A)') 'Adjoint output by BuFlowModule'
    WRITE(fileunit, '(A)') 'ASCII'
    WRITE(fileunit, '(A)') 'DATASET UNSTRUCTURED_GRID'
    WRITE(fileunit, '(A,I0,A)') 'POINTS ', npts, ' double'
    DO i=1,npts
    ...
    WRITE(fileunit, '(A)') 'LOOKUP_TABLE default'
    DO i=1,ncells
      WRITE(fileunit, '(F16.8)') primitives(i, 1)
    END DO
    WRITE(fileunit, '(A)') 'SCALARS T double 1'
    WRITE(fileunit, '(A)') 'LOOKUP_TABLE default'
    DO i=1,ncells
      WRITE(fileunit, '(F16.8)') primitives(i, 2)
    END DO
    WRITE(fileunit, '(A)') 'VECTORS U double'
    DO i=1,ncells
      WRITE(fileunit, '(3F16.8)') primitives(i, 3), primitives(i, 4), primitives(i, 5)
    END DO
    WRITE(fileunit, '(A)') 'SCALARS J double 1'
    WRITE(fileunit, '(A)') 'LOOKUP_TABLE default'
    DO i=1,ncells
      WRITE(fileunit, '(ES16.8)') drag_value
    END DO
    WRITE(fileunit, '(A)') 'FIELD adjoint_J_p 1'
    WRITE(fileunit, '(A)') 'dJdp 4 1'
    WRITE(fileunit, '(4(1X,ES24.16))') dJdp(1), dJdp(2), dJdp(3), dJdp(4)
    WRITE(fileunit, '(A)') 'FIELD df_dx 1'
    WRITE(fileunit, '(A)') 'dJdp_partial 4 1'
    WRITE(fileunit, '(4(1X,ES24.16))') dJdp_partial(1), dJdp_partial(2), dJdp_partial(3), dJdp_partial(4)
    WRITE(fileunit, '(A)') 'SCALARS dJ_dP double 1'
    WRITE(fileunit, '(A)') 'LOOKUP_TABLE default'
    DO i=1,ncells
      WRITE(fileunit, '(ES16.8)') dJdw(i, 1)
    END DO
    WRITE(fileunit, '(A)') 'SCALARS dJ_dT double 1'
    WRITE(fileunit, '(A)') 'LOOKUP_TABLE default'
    DO i=1,ncells
      WRITE(fileunit, '(ES16.8)') dJdw(i, 2)
    END DO
    WRITE(fileunit, '(A)') 'VECTORS dJ_dU double'
    DO i=1,ncells
      WRITE(fileunit, '(3ES16.8)') dJdw(i, 3), dJdw(i, 4), dJdw(i, 5)
    END DO
    CLOSE(fileunit)
  END SUBROUTINE WRITEVTKFILE_B
...
!   gradient     of useful results: *fluxresiduals
!   with respect to varying inputs: *fluxresiduals data_4d137 cellprimitives
!   RW status of diff variables: *epss:(loc) *point_update:(loc)
!                *cellstate_sln:(loc) *fcs:(loc) *ccenters_mesh:(loc)
!                *facepts:(loc) *fluxresiduals:in-out *eps22:(loc)
...
!                *cellprimitives_sln:(loc) *fdeltas:(loc) data_4d137:out
!                cellprimitives:out
...
&   cellprimitives, cellprimitivesb)
    IMPLICIT NONE
...
!		allocate(fluxResidualss(nCells, 5), source=0.0_8)
  ...
  SUBROUTINE COMPUTE_DRAG_B(data_4d137, cellprimitives, cellprimitivesb&
&   , j, jb)
    USE TYPESMODULE_DIFF
    IMPLICIT NONE
    REAL(kind=8), INTENT(IN) :: data_4d137(1, 4)
    ...
    print *, "ABC"
    cellprimitivesb = 0.0_8
    IF ((.NOT.ALLOCATED(boundarynumfacess)) .OR. (.NOT.ALLOCATED(&
&       boundaryfaces_mesh))) THEN
      jb = 0.0_8
      RETURN
    END IF
    IF (SIZE(boundarynumfacess) .LT. 1) THEN
      jb = 0.0_8
      RETURN
    END IF
    IF ((.NOT.ALLOCATED(faces_mesh)) .OR. (.NOT.ALLOCATED(favecs_mesh))) THEN
      jb = 0.0_8
      RETURN
    END IF
    print *, "ABC"
    ...
    IF (nwall .LE. 0) THEN
      ...
      RETURN
    END IF
    IF (SIZE(boundaryfaces_mesh, 2) .LT. nwall) THEN
      print *, "Ad"
      jb = 0.0_8
      print *, "Ad"
      RETURN
    ...
    DO k=nwall,1,-1
      ...
      IF (branch .NE. 0) THEN
        face_idx = boundaryfaces_mesh(1, k)
        IF (face_idx .GE. 1 .AND. face_idx .LE. nf) THEN
          owner_cell = faces_mesh(face_idx, 1)
          IF (owner_cell .GE. 1 .AND. owner_cell .LE. nc) THEN
            cellprimitivesb(owner_cell, 1) = cellprimitivesb(owner_cell, 1&
&             ) - favecs_mesh(face_idx, 1)*jb
          END IF
        END IF
      END IF
    END DO
  ...
  subroutine compute_drag(data_4d137, cellPrimitives, J)
	...
	integer(kind=8) :: nWall, k, face_idx, owner_cell, nf, nc
	J = 0.0_8
	if (.not. allocated(boundarynumfacess)) return
	if (.not. allocated(boundaryfaces_mesh)) return
	if (size(boundarynumfacess) < 1) return
	if (.not. allocated(faces_mesh) .or. .not. allocated(favecs_mesh)) return
	nf = size(faces_mesh, 1)
	nc = size(cellPrimitives, 1)
	nWall = boundarynumfacess(1)
	if (nWall <= 0) then
	...
	if (size(boundaryfaces_mesh, 2) < nWall) return
	do k = 1, nWall
		face_idx = boundaryfaces_mesh(1, k)
		if (face_idx <= 0 .or. face_idx > nf) cycle
		owner_cell = faces_mesh(face_idx, 1)
		if (owner_cell < 1 .or. owner_cell > nc) cycle
		J = J - cellPrimitives(owner_cell, 1) * favecs_mesh(face_idx, 1)
	end do
	drag_value = J
  end subroutine compute_drag
   
!! JF-GMRES 求解 (∂R/∂w)^T psi = b. 矩阵-向量积由外部 matvec_at 提供（_b 中为反向 AD）.
  subroutine solve_adjoint_jfgmres(b, psi, n, maxit, tol, matvec_at, niter, resnorm)
		...
		print '(A,ES14.6,A,ES14.6,A,ES14.6,A,ES14.6)', ' [JF-GMRES] ||b||=', &
		bnrm, ' tol*||b||=', tol_b, ' ||r0||=', rnrm, ' ||r0||/||b||=', rnrm/bnrm
		m = 0
		do k = 1, maxit
			v(:, 1) = r / rnrm
			e1 = 0.0_8
			...
			print '(A,I0,A,ES14.6,A,ES14.6,A,L1)', ' [JF-GMRES] outer=', k, &
			' ||r||=', rnrm, ' ||r||/||b||=', rnrm/bnrm, ' converged=', (rnrm <= tol_b)
			if (rnrm <= tol_b) exit
			r = matmul(v(1:n, 1:m+1), e1(1:m+1))
			rnrm = sqrt(sum(r*r))
		end do
		if (present(niter)) niter = k
		print '(A,I0,A,ES14.6)', ' [JF-GMRES] done: niter=', k, ' final ||r||=', rnrm
