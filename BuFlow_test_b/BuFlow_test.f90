...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		print_eps = 5  ! 最多输出5个面，避免越界
		PRINT *, NEW_LINE('a')//'=== 前5个面的eps值（二阶/四阶） ==='
		PRINT *, '格式：面索引 | eps22（二阶） | eps44（四阶）'
		DO f = 1, print_eps
			WRITE(*, '(I5, 2ES14.6)') f, epss(f, 1), epss(f, 2)  ! 用科学计数法适配可能的小数值
		END DO
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	
		eps22 = epss(:,1)  ! 二阶耗散系数
		eps44 = epss(:,2)  ! 四阶耗散系数
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		print_resd = 5
		PRINT *, NEW_LINE('a')//'=== 前5个单元的 fluxresidualsd（微分残差） ==='
...
!		END DO
		...
		print_slnd = 5
		PRINT *, NEW_LINE('a')//'=== 前5个单元的 slnd%cellstate（微分状态变量） ==='
...
! 11.NACA0012.jl!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	! 变量赋值
	!-----------------------------------------------------------------------------------#
	subroutine compute_CFD(cellPrimitivess, cellPrimitivess_1)
		type(Fluidd) :: fluid
